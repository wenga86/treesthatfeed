...
AccessControl.map :require => [ :admin, :publisher, :contributor ]  do |map|
  ...
  map.project_module :albums, nil do |project|
    project.menu "Albums",              { :controller => "admin/albums", :action => "index"}
    project.submenu "All Albums",   { :controller => "admin/albums", :action => "index" }
  end
