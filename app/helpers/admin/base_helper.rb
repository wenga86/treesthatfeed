module Admin::BaseHelper
  ...
  def subtabs_for(current_module)
    ...
    AccessControl.project_module(current_user.profile_label, current_module).submenus.each_with_index do |m,i|
      ...
      current =
      output << subtab(_(m.name), (m.url[:controller] == params[:controller] && m.url[:action] == params[:action]) ? '' : m.url) unless m.name == "Choose theme"
    end
    output.join("\n").html_safe
  end
  ...
  def show_thumbnail_for_editor(image, klass = false)
    ...
    picture << "<img class='tumb#{' img-polaroid' if klass}' src='#{thumbnail}' "
